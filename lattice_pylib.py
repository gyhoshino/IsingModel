...
class IsingLattice(object):
    ...
    def get_site_flip_prob(self, i, j, T, B):
        ...
        # if self.DeltaE is None:
        self.DeltaE = 2.0 * (self.J*(self.spin*self.neighbors) + float(self.B)*self.spin)
        if self.DeltaE[i, j] < 0:
            return 1.
        else:
            return np.exp(-1.0*self.DeltaE[i,j]/float(T))
    ...
    def step(self, T, B):
        ...
        p_trans = np.where(self.DeltaE >= 0.0, np.exp(-1.0*self.DeltaE/float(T)),1.0)
